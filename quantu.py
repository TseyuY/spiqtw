	...
	try:
		for center in bsObj.findAll("div",{"class":"c_page"}):
			...
			for a in center.findAll("a"):
				...
				llink = 'http://www.quantuwang.co'+str(a.attrs['href'])
	...
	for boxs in bsObj.findAll("div", {"class":"index_left"}): # 找到装载图片集的<div>标签
		...
		for li in boxs.findAll("li"):   
			...
			for a in li.findAll('a'): # 找到包含图片链接的p标签
                #print(a.attrs['href'])
                #psn = p.find('a')
				package_list.append('http://www.quantuwang.co'+str(a.attrs['href'])) # 用文件名作为key给字典添加图集链接
    #print(package_list)
...
'''
PACKAGE_NUM = input("Enter the num: ")
